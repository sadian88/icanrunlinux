            ...
            for i, rank_th in enumerate(ranks[:50]):
                try:
                    ...
                    if name_td:
                        ...
                        link = name_td.find('a')
                        slug = link.get('href', name.lower().replace(' ', '-')) if link else name.lower().replace(' ', '-')  # noqa: E501
...
def extract_distro_details(slug: str) -> Optional[dict]:
    ...
    h1 = soup.find('h1')
    if h1:
        details['name'] = h1.get_text(strip=True).replace('Table of Distribution ', '').replace('Table of Distribution ', '')  # noqa: E501
